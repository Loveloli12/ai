...
const getPageList = () => {
    ...
    getKnowledgeList(params).then(res => {
        ...
        if (res.records && res.records.length > 0) {
            console.log('coverImage:', res.records[0].coverImage)
            console.log('getImage result:', getImage(res.records[0].coverImage))
        }
    })
}
//获取封面图片
const getImage = (url) => {
    if (!url || url === 'none' || url.trim() === '') {
        return new URL('@/assets/images/book.png', import.meta.url).href
    }
    if (url.startsWith('http://') || url.startsWith('https://')) {
        const path = url.replace('http://159.75.169.224:1235', '').replace('https://159.75.169.224:1235', '')
        return '/files' + path
    }

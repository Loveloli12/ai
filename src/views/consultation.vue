    ...
    <div class="consultation-container">
        <div class="sidebar">
            <!-- AI助手信息 -->
             <div class="ai-assistant-info">
                ...
                <h3 class="assistant-name">雌小鬼</h3>
                        ...
                        <div class="session-info">
                            <div class="session-title">
                                ...
                                <div class="session-meta">
                                    <span class="session-time">{{ session.createdAt }}</span>
                                </div>
        ...
        <div class="chat-main">
            ...
            <div class="chat-messages">
                <!-- 欢迎用语 -->
                <div v-if="messages.length === 0" class="message-item ai-messasge">
                    <div class="message-avatar">
                        <el-image :src="iconUrl" style="width:18px; height: 18px;"/>
                    </div>
                    <div class="message-content">
                        <div class="message-bubble">
                            <p>你好，我是主人的雌小鬼，我可以嘲笑你并扩大你的问题，也可以给你的心情捣乱。</p>
                        </div>
                ...
                <div v-for="msg in messages" :key="msg.id" class="message-item" :class="msg.senderType === 1? 'user-message': 'ai-message'">
                    ...
                    <div class="message-content">
                        <div class="message-bubble">
                            ...
                            <!-- AI错误提示 -->
                            <div v-else-if="msg.isError" class="erroe-message">
                                <p>{{ msg.content }}</p>
                            </div>
                            <!-- 正常消息 -->
                        ...
                        <div class="message-time">{{ msg.senderType === 2 && isAiTying ? '正在打字骂你...' : msg.createdAt }}</div>
                    </div>
                </div>
            </div>
            <!-- 输入框 -->
...
const startNewSession = (message) => {
    ...
    if (currentSession.value && currentSession.value.sessionTitle === '新会话') {
        sessionParams.sessionTitle = `雌小鬼 - ${new Date().toLocaleString()}`
    } else {
        //如果是历史会话记录
...
const startAIResponse = (sessionId,userMessage) => {
    ...
    //调用流式接口
    const ctrl = new AbortController() //用于取消fetch请求
    fetchEventSource('/api/psychological-chat/stream',{
        ...
        onmessage:(event) => {
            ...
            if(ok && payload.data && payload.data.content){
                ...
                //错误回复显示
                handleError(payload.message || '小鬼懵圈')
            }
        },
        onError:(err) => {
            handleError(err.message || '小鬼懵圈')
            throw err
        },
        onClose:() => {
            //开始情绪分析
...
const handleError = (error) => {
    ...
    const aiMessage = messages.value[messages.value.length - 1]
    if(aiMessage) {
        aiMessage.content = '小鬼懵圈，无法回答'
    }
    isAiTying.value = false
    ElMessage.error('小鬼懵圈，无法回答')
}
...
const handleDeleteSession = (sessionId) => {
    ...
    deleteSession(sessionId).then(res => {
            ...
            getSessionsPage()
            if (currentSession.value && (currentSession.value.sessionId === sessionId || currentSession.value.sessionId === `@session_${sessionId}`)) {
